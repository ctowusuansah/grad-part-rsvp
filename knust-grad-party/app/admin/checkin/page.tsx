...
export default function CheckinPage() {
  ...
  async function search(q: string) {
    ...
    const res = await fetch(`/api/admin/rsvps?q=${encodeURIComponent(q)}`);
    if (res.status === 401) {
  ...
  async function checkIn(id: string, override = false) {
    ...
    if (res.ok) {
      ...
      const confirmOverride = window.confirm(
        `${data.record?.full_name || "This attendee"} is already checked in. Check in again anyway?`
      );
      if (confirmOverride) checkIn(id, true);
    } else {
  ...
  async function startScan() {
    ...
    try {
      ...
      setMessage("Could not access the camera. Use search instead.");
      setScanning(false);
    }
  }
  ...
  return (
    <main className="mx-auto max-w-md space-y-6 px-4 py-8">
      <h1 className="font-display text-xl font-bold uppercase text-cream">Check-in</h1>
      ...
      {message && <p className="rounded-sm bg-cream px-3 py-2 text-sm font-semibold text-ink">{message}</p>}

      <div className="space-y-2">
        {results.map((r) => (
          <div key={r.id} className="case-card flex items-center justify-between rounded-sm px-4 py-3">
            <div>
              <p className="font-semibold text-ink">{r.full_name}</p>
              <p className="text-xs text-ink/60">
                {formatRsvpId(r.serial)} · {r.payment_status} · {r.checked_in ? "Checked in" : "Not checked in"}
              </p>
            </div>
            <button
