  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; color: #231a14;">
        ...
        <p>Your payment will be manually verified by the organizing committee.
        You'll receive another email once it's confirmed. Please keep your RSVP ID
        — you'll need it (as a QR code) to check in at the event.</p>
        <p>See you there!</p>
      </div>
    `,
  });
...
export async function sendConfirmedEmail(args: { toEmail: string; toName: string; serial: number }) {
  const resend = getResend();
  if (!resend) {
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; color: #231a14;">
        <h2>${EVENT_NAME}</h2>
        <p>Hi ${args.toName},</p>
        <p><strong>Your payment has been confirmed and your RSVP is officially confirmed.</strong></p>
        <p>Your RSVP ID: <strong>${rsvpId}</strong></p>
        <p>Date: ${EVENT_DATE_DISPLAY}<br/>
        Venue: ${EVENT_VENUE}<br/>
        Time: ${EVENT_TIME_DISPLAY}</p>
        <p>Bring your RSVP confirmation (with its QR code) to check in at the entrance.</p>
        <p>See you there!</p>
      </div>
    `,
  });
