...
export default function AdminDashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (rsvpFilter !== "all") params.set("rsvp_status", rsvpFilter);
  ...
  async function updateStatus(id: string, patch: Record<string, string>) {
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-display text-2xl font-bold uppercase text-cream">Admin Dashboard</h1>
        <div className="flex gap-2">
          ...
          <button onClick={logout} className="rounded-sm bg-accent px-4 py-2 text-sm font-semibold text-cream">
            Log Out
          </button>
        </div>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <StatCard label="Payment Confirmed" value={stats.payment_confirmed} />
          <StatCard label="Payment Pending" value={stats.payment_pending} />
          <StatCard label="Payment Rejected" value={stats.payment_rejected} />
          <StatCard label="Total Expected" value={`GHS ${stats.total_expected}`} />
          <StatCard label="Total Received" value={`GHS ${stats.total_received}`} />
          <StatCard label="Total Outstanding" value={`GHS ${stats.total_outstanding}`} />
        </div>
      )}
          ...
          <tbody>
            ...
            {records.map((r) => (
              <tr key={r.id} className="border-b border-ink/10">
                <td className="px-3 py-2 font-mono">{formatRsvpId(r.serial)}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-sm bg-cream p-6">
            <div className="flex items-start justify-between">
              ...
              <button onClick={() => setSelected(null)} className="text-ink/60">
                ✕
              </button>
            </div>
            ...
            <div className="mt-4 space-y-2 text-sm text-ink">
              ...
              <Detail label="Amount Due" value={`GHS ${selected.amount_due}`} />
              <Detail label="Amount Submitted" value={`GHS ${selected.amount_submitted ?? "—"}`} />
              <Detail label="MoMo Transaction Ref" value={selected.momo_transaction_ref || "—"} />
              <Detail label="MoMo Payer Name" value={selected.momo_payer_name || "—"} />
              <Detail label="Payment Status" value={selected.payment_status} />
              <Detail label="RSVP Status" value={selected.rsvp_status} />
              <Detail label="Checked In" value={selected.checked_in ? `Yes (${new Date(selected.checked_in_at || "").toLocaleString()})` : "No"} />
            </div>

            {screenshotUrl && (
              <div className="mt-4">
                <p className="mb-1 text-xs font-semibold uppercase text-ink/60">Payment Screenshot</p>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={screenshotUrl} alt="Payment screenshot" className="max-h-64 rounded-sm border" />
              </div>
            )}

            <div className="mt-6 flex flex-wrap gap-2">
              <button
                onClick={() => updateStatus(selected.id, { payment_status: "confirmed" })}
                className="rounded-sm bg-green-700 px-4 py-2 text-sm font-semibold text-cream"
              >
                Confirm Payment
              </button>
              <button
                onClick={() => updateStatus(selected.id, { payment_status: "rejected" })}
                className="rounded-sm bg-accent px-4 py-2 text-sm font-semibold text-cream"
              >
                Reject Payment
              </button>
              <button
                onClick={() => updateStatus(selected.id, { payment_status: "pending" })}
              ...
              {selected.rsvp_status === "active" ? (
                <button
                  onClick={() => updateStatus(selected.id, { rsvp_status: "cancelled" })}
                ...
                <button
                  onClick={() => updateStatus(selected.id, { rsvp_status: "active" })}
                  className="rounded-sm border border-ink/30 px-4 py-2 text-sm font-semibold text-ink"
                >
                  Reactivate RSVP
                </button>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`rounded-sm px-2 py-1 text-xs font-semibold ${colors[status] || ""}`}>
      {status}
    </span>
  );
}
