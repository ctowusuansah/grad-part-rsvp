...
export default async function ConfirmationPage({ params }: { params: { id: string } }) {
  ...
  const qrPayload = record.id; // QR encodes the unique RSVP id, scanned at check-in.
  ...
  return (
    <main className="mx-auto max-w-md space-y-6 px-4 py-10">
      <div className="case-card mx-auto max-w-md rounded-sm px-6 py-8 text-center">
        <div className="pin" />
        <h1 className="font-display text-2xl font-bold uppercase text-ink">RSVP Received</h1>
        ...
        <div className="mt-6 space-y-2 text-left text-sm text-ink">
          ...
          <Row label="Contribution" value="GHS 40" />
          <Row
        ...
        <p className="mt-6 text-xs text-ink/60">
          Save this page or screenshot your QR code — you'll present it at the entrance to check in.
...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex justify-between border-b border-ink/10 pb-1">
      ...
      <span className={highlight ? "font-semibold text-accent" : "font-semibold"}>{value}</span>
    </div>
  );
}
