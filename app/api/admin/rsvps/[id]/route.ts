...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: record, error } = await supabase.from("rsvps").select("*").eq("id", params.id).single();
  if (error || !record) {
  ...
  if (record.payment_screenshot_path) {
    const { data: signed } = await supabase.storage
      .from("payment-proofs")
      .createSignedUrl(record.payment_screenshot_path, 60 * 10); // 10 minutes
    screenshotUrl = signed?.signedUrl || null;
  }

  return NextResponse.json({ record, screenshotUrl });
}
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body.payment_status && ["pending", "confirmed", "rejected"].includes(body.payment_status)) {
    update.payment_status = body.payment_status;
  }
  if (body.rsvp_status && ["active", "cancelled"].includes(body.rsvp_status)) {
  ...
  if (update.payment_status === "confirmed" && !record.confirmed_email_sent) {
    try {
      ...
      if (result.sent) {
        await supabase.from("rsvps").update({ confirmed_email_sent: true }).eq("id", record.id);
      }
    } catch (e) {
